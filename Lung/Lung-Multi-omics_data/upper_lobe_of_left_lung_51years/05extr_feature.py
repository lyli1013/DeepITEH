...
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_51years/H3K4me1-ENCFF007KEH.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_51years/H3K4me3-ENCFF454DNT.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_51years/H3K9me3-ENCFF639TLJ.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_51years/H3K27ac-ENCFF768STB.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_51years/H3K36me3-ENCFF685YYP.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [450, 750, 300, 550, 200]
featurename = '/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_51years/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Lung/lung_enhancer6481_6001.fa", "fasta"):
        ...
        if int(window_extend_size) == 200:
            start_position = start_position + 400
            end_position = end_position + 400
            value_one7 = [0]
            value_sum7 = 0
            chromStartlist7 = [0, 0]
            chromEndlist7 = [0, 0]
...
np.savetxt(featurename + 'en_feature_6481_51years.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
