...
with open("/data/lyli/Lung/Lung-Multi-omics_data/lower_lobe_of_left_lung_59years_hg38/hg19/H3K4me1-ENCFF479WUX.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/lower_lobe_of_left_lung_59years_hg38/hg19/H3K4me3-ENCFF180MOQ.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/lower_lobe_of_left_lung_59years_hg38/hg19/H3K9me3-ENCFF180RTK.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/lower_lobe_of_left_lung_59years_hg38/hg19/H3K27ac-ENCFF011IHJ.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/lower_lobe_of_left_lung_59years_hg38/hg19/H3K36me3-ENCFF769ZRI.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [600, 900, 550, 750, 800]
featurename = '/data/lyli/Lung/Lung-Multi-omics_data/lower_lobe_of_left_lung_59years_hg38/hg19/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
H3K27ac_value_sum5 = []
H3K36me3_value_sum7 = []
#for record in SeqIO.parse("/data/lyli/deep-dataset/enhancer_all/enhancer_train/enhancer_train2000.fa", "fasta"):
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Lung/lung_enhancer6481_6001.fa", "fasta"):
        name = record.id
...
np.savetxt(featurename + 'en_feature_6481_59year.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
