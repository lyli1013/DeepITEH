...
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_54years/H3K4me1-ENCFF294XHM.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_54years/H3K4me3-ENCFF285TGB.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_54years/H3K9me3-ENCFF130BDL.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_54years/H3K27ac-ENCFF045KTS.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_54years/H3K36me3-ENCFF916CFG.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [400, 700, 500, 450, 300]
featurename = '/data/lyli/Lung/Lung-Multi-omics_data/upper_lobe_of_left_lung_54years/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Lung/lung_enhancer6481_6001.fa", "fasta"):
        ...
        if int(window_extend_size) == 300:
            start_position = start_position + 600
            end_position = end_position + 600
            value_one7 = [0]
            value_sum7 = 0
            chromStartlist7 = [0, 0]
            chromEndlist7 = [0, 0]
...
np.savetxt(featurename + 'en_feature_6481_54years.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
