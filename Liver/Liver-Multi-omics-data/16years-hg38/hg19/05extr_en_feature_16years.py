...
with open("/data/lyli/deep-dataset/Multi-omics_data/16years_right_liver_hg38/01/hg19/H3K4me1.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/deep-dataset/Multi-omics_data/16years_right_liver_hg38/01/hg19/H3K4me3.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/deep-dataset/Multi-omics_data/16years_right_liver_hg38/01/hg19/H3K9me3.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/deep-dataset/Multi-omics_data/16years_right_liver_hg38/01/hg19/H3K27ac.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/deep-dataset/Multi-omics_data/16years_right_liver_hg38/01/hg19/H3K36me3.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [550, 800, 400, 650]
featurename = '/data/lyli/deep-dataset/Multi-omics_data/16years_right_liver_hg38/01/hg19/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/deep-dataset/enhancer_all/enhancer6001.fa", "fasta"):
        name = record.id
...
np.savetxt(featurename + 'en_feature_3171_16year_01.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
