...
with open("/data/lyli/Liver/Liver-Multi-omics-data/31years/H3K27ac.bed") as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())  # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开



featurename = '/data/lyli/Liver/Liver-Multi-omics-data/31years/Enhancer_peak_distribution/H3K27ac/'
w = 1
mult = ['window01.fa', 'window02.fa', 'window03.fa', 'window04.fa', 'window05.fa', 'window06.fa', 'window07.fa']
for j in mult:
    have_peak_enhancer_nums = 0
    enhancers_feature = []
    for record in SeqIO.parse("/data/lyli/Liver/liver-enhancers-windows/enhancer-900/" + j, "fasta"):
        location_total = []
        distance_total = []
        signalValue_total = []
        name = record.id
