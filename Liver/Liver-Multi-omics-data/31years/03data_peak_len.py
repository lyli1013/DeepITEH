...
for i in omics_data:
    print("这是组学数据：", i)
    content_liver_omics = []
    with open("/data/lyli/deep-dataset/Multi-omics_data/31years/01/" + i + ".bed")as f:
        for line in f:
    ...
    newfile = open("/data/lyli/deep-dataset/Multi-omics_data/31years/01/omics_data_len/" + i + ".txt", 'w')
    for a in omics_len:
        newfile.write(str(a) + "\n")
    newfile.close()