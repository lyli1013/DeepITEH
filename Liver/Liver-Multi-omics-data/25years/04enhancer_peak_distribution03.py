...
with open("/data/lyli/Liver/Liver-Multi-omics-data/25years/H3K27ac.bed") as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())  # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开



featurename = '/data/lyli/Liver/Liver-Multi-omics-data/25years/Enhancer_peak_distribution/H3K27ac/'
w = 1
...
for j in mult:
    ...
    enhancers_feature = []
    for record in SeqIO.parse("/data/lyli/Liver/liver-enhancers-windows/enhancer-700/" + j, "fasta"):
        location_total = []
        distance_total = []
        signalValue_total = []
        name = record.id
