...
for w in windows:
    ...
    checkpoint_dir = '/data/lyli/Liver/liver-model/Seq_feature_exact/window' + w  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Liver/liver-model/Seq_feature_exact/window' + w + '/checkpoints'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Liver/liver-model/Seq_feature_exact/window' + w + '/checkpoints/final_seq_model'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)
    ...
    # load bert model
    config_path = '/data/lyli/Liver/liver-model/Bert-embedding/cased_L-12_H-768_A-12/bert_config.json'
    checkpoint_path = '/data/lyli/Liver/liver-model/Bert-embedding/cased_L-12_H-768_A-12/bert_model.ckpt'
    dict_path = '/data/lyli/Liver/liver-model/Bert-embedding/cased_L-12_H-768_A-12/vocab.txt'
    model_bert = load_trained_model_from_checkpoint(config_path, checkpoint_path)

    # load sequences train-data
    filename1 = "/data/lyli/Liver/liver-enhancers-windows/enhancer-300/window" + w + "_train_2537.txt"
    filename2 = "/data/lyli/Liver/NoEnhancer/train_nenseq2537_300.txt"
    ...
    # load vocabulary of bert, and store to dictionary
    token_dict = {}
    ...
    # update dictionary of vocabulary
    n = 1
    ...
    np.save('/data/lyli/Liver/liver-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy', token_dict)

    tokenizer = OurTokenizer(token_dict)   # divide sentences
    X_seq_train = [extract_single_features(tokenizer, x['sequence']) for x in sequenceList]    # get word vector
    ...
    # evaluation model
    filename3 = "/data/lyli/Liver/liver-enhancers-windows/enhancer-300/window" + w + "_test_634.txt"
    filename4 = "/data/lyli/Liver/NoEnhancer/test_nenseq634_300.txt"
    ...
    # Load SeqPoseDict
    token_dict = np.load('/data/lyli/Liver/liver-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy',
