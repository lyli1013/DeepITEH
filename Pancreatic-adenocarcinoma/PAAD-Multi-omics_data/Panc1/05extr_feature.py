...
with open("/data/lyli/Pancreatic-adenocarcinoma/PAAD-Multi-omics_data/Panc1/H3K4me1-ENCFF096OQW.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Pancreatic-adenocarcinoma/PAAD-Multi-omics_data/Panc1/H3K4me3-ENCFF604STS.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Pancreatic-adenocarcinoma/PAAD-Multi-omics_data/Panc1/H3K9me3-ENCFF341CNZ.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Pancreatic-adenocarcinoma/PAAD-Multi-omics_data/Panc1/H3K27ac-ENCFF087IKD.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Pancreatic-adenocarcinoma/PAAD-Multi-omics_data/Panc1/H3K36me3-ENCFF126YLO.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [300, 800, 250, 500, 150]
featurename = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-Multi-omics_data/Panc1/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Pancreatic-adenocarcinoma/PAAD-enhancers835-hg19.fa", "fasta"):
        name = record.id
        ...
        if int(window_extend_size) == 250:
            start_position = start_position + 500
            end_position = end_position + 500
            value_one4 = [0]
            value_sum4 = 0
            chromStartlist4 = [0, 0]
            chromEndlist4 = [0, 0]
        ...
        if int(window_extend_size) == 150:
            start_position = start_position - 1800
            end_position = end_position - 1800
            value_one7 = [0]
            value_sum7 = 0
            chromStartlist7 = [0, 0]
            chromEndlist7 = [0, 0]
...
np.savetxt(featurename + 'en_feature_835_Panc1.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
