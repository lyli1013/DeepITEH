...
for window in windows:
    enhancers = []
    for record in SeqIO.parse("/data/lyli/Pancreatic-adenocarcinoma/PAAD-enhancers-windows/PAAD-enhancers-300bp/" + window + ".fa", "fasta"):
        name = record.id
        enhancers.append(name)

    newfile1 = open("/data/lyli/Pancreatic-adenocarcinoma/PAAD-enhancers-windows/PAAD-enhancers-300bp/" + window + "_train_835.txt", 'w')
    random.seed(1)
    ...
    newfile2 = open("/data/lyli/Pancreatic-adenocarcinoma/PAAD-enhancers-windows/PAAD-enhancers-300bp/" + window + "_test_167.txt", 'w')
    for i in test_enhancers:
        chromosome = i.split(":")[0]
        chromosome = str(chromosome)
        # print(chromosome)
