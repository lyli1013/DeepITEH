...
k_merList = [2]  # kmer
kkk = k_merList[0]
BATCH_SIZE = 32   # best_parameter
EPCOHS = 7         # best_parameter
...
checkpoint_dir = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Bert_LSTM_DNN_model/Panc1/final_model_panc1'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

model_weight_filepath = checkpoint_dir + "/PAAD_panc1_model.h5"  # First layer model
model_structure_filepath = checkpoint_dir + "/PAAD_panc1_model.json"  # First layer model
...
def Model_AUC(model, X_test_first, Y_test_first, BATCH_SIZE):
    ...
    dataframe.to_csv("/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Bert_LSTM_DNN_model/Panc1/PAAD_panc1_method.csv", index=False, sep=',')
...
filename1 = "/data/lyli/Pancreatic-adenocarcinoma/Processed_dataset-new/enhancer/en_feature_Panc1_RE_AE_test167.txt"  # 测试集
filename2 = "/data/lyli/Pancreatic-adenocarcinoma/Processed_dataset-new/nenhancer/nen_feature_Panc1_RE_AE_test167.txt"
filename3 = "/data/lyli/Pancreatic-adenocarcinoma/Processed_dataset-new/enhancer/test_enseq167.txt"
...
# load bert model
config_path = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Bert-embedding/cased_L-12_H-768_A-12/bert_config.json'
checkpoint_path = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Bert-embedding/cased_L-12_H-768_A-12/bert_model.ckpt'
dict_path = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Bert-embedding/cased_L-12_H-768_A-12/vocab.txt'
model_bert = load_trained_model_from_checkpoint(config_path, checkpoint_path)

# Load SeqPoseDict
token_dict = np.load('/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Bert-embedding/paad_seq_Dict.npy', allow_pickle=True).item()
