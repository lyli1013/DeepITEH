...
for w in windows:
    ...
    checkpoint_dir = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Seq_feature_exact/window' + w  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Seq_feature_exact/window' + w + '/checkpoints'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Seq_feature_exact/window' + w + '/checkpoints/final_seq_model'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    output_dir = '/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Seq_feature_exact/window' + w + '/output'  # Store model files
    ...
    filename1 = "/data/lyli/Pancreatic-adenocarcinoma/PAAD-enhancers-windows/PAAD-enhancers-300bp/window" + w + "_train_668.txt"
    filename2 = "/data/lyli/Pancreatic-adenocarcinoma/Processed_dataset-new/nenhancer/train_nenseq668.txt"
    ...
    for pairs in wordVectorSeqList:
        for token in pairs:
            if token not in token_dict:
                ...
                n += 1
    # print(token_dict)
    np.save('/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy', token_dict)
    ...
    # evaluation model
    filename3 = "/data/lyli/Pancreatic-adenocarcinoma/PAAD-enhancers-windows/PAAD-enhancers-300bp/window" + w + "_test_167.txt"
    ...
    # Load SeqPoseDict
    token_dict = np.load('/data/lyli/Pancreatic-adenocarcinoma/PAAD-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy',
