...
checkpoint_dir = '/data/lyli/Stomach/stomach-model/Bert_LSTM_DNN_model/54years/final_model_54years'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

model_weight_filepath = checkpoint_dir + "/stomach_54years_model.h5"  # First layer model
model_structure_filepath = checkpoint_dir + "/stomach_54years_model.json"  # First layer model
...
# load train sequence's word vector
X_seq_train = np.loadtxt('/data/lyli/Stomach/stomach-model/Bert-embedding/stomach_seq_train_6308_wordVector.txt', delimiter=',').reshape((6308, 512, 768))
X_seq_train = np.array(X_seq_train)

# load omics feature
# training dataset
filename1 = "/data/lyli/Stomach/Processed_dataset/enhancer/en_feature_54years_RE_AE_train3154.txt"
filename2 = "/data/lyli/Stomach/Processed_dataset/nenhancer/nen_feature_54years_RE_AE_train3154.txt"
