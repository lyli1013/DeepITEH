...
for w in windows:
    ...
    checkpoint_dir = '/data/lyli/Stomach/stomach-model/Seq_feature_exact/window' + w  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Stomach/stomach-model/Seq_feature_exact/window' + w + '/checkpoints'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Stomach/stomach-model/Seq_feature_exact/window' + w + '/checkpoints/final_seq_model'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    output_dir = '/data/lyli/Stomach/stomach-model/Seq_feature_exact/window' + w + '/output'  # Store model files
    if not os.path.exists(output_dir):  # if the path does not exist, generate it
        os.mkdir(output_dir)
    filepath = checkpoint_dir + "/stomach_seq_model_" + str(kkk) + "_mer" + ".hdf5"  # First layer model
    ...
    filename1 = "/data/lyli/Stomach/stomach-enhancers-windows/stomach-enhancers-300bp/window" + w + "_train_3154.txt"
    filename2 = "/data/lyli/Stomach/Processed_dataset/nenhancer/window11_train_3154.txt"
    ...
    for pairs in wordVectorSeqList:
        for token in pairs:
            if token not in token_dict:
                ...
                n += 1
    # print(token_dict)
    np.save('/data/lyli/Stomach/stomach-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy', token_dict)
    ...
    # evaluation model
    filename3 = "/data/lyli/Stomach/stomach-enhancers-windows/stomach-enhancers-300bp/window" + w + "_test_789.txt"
    ...
    # Load SeqPoseDict
    token_dict = np.load('/data/lyli/Stomach/stomach-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy',allow_pickle=True).item()
