...
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-54years/H3K4me1-ENCFF334NQD.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-54years/H3K4me3-ENCFF302NRA.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-54years/H3K9me3-ENCFF776FZW.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-54years/H3K27ac-ENCFF058UTH.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-54years/H3K36me3-ENCFF510USZ.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [350, 700, 200, 550, 200]
featurename = '/data/lyli/Stomach/stomach-Multi-omics_data/stomach-54years/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
np.savetxt(featurename + 'en_feature_3943_54years.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
