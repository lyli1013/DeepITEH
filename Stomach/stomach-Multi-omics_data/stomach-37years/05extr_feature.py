...
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-37years/H3K4me1-ENCFF383JJS.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-37years/H3K4me3-ENCFF005YAO.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-37years/H3K9me3-ENCFF108LMS.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-37years/H3K27ac-ENCFF811JFI.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Stomach/stomach-Multi-omics_data/stomach-37years/H3K36me3-ENCFF392RTZ.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [600, 800, 200, 650, 900]
featurename = '/data/lyli/Stomach/stomach-Multi-omics_data/stomach-37years/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Stomach/stomach_enhancers_3943.fa", "fasta"):
        name = record.id
...
np.savetxt(featurename + 'en_feature_3943_37years.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
