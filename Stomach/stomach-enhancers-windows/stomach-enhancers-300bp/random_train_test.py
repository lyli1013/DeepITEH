...
for window in windows:
    enhancers = []
    for record in SeqIO.parse("/data/lyli/Stomach/stomach-enhancers-windows/stomach-enhancers-300bp/" + window + ".fa", "fasta"):
        name = record.id
        enhancers.append(name)

    newfile1 = open("/data/lyli/Stomach/stomach-enhancers-windows/stomach-enhancers-300bp/" + window + "_train_3154.txt", 'w')
    random.seed(1)
    test_enhancers = random.sample(enhancers, 789)
    ...
    newfile2 = open("/data/lyli/Stomach/stomach-enhancers-windows/stomach-enhancers-300bp/" + window + "_test_789.txt", 'w')
    for i in test_enhancers:
        chromosome = i.split(":")[0]
        chromosome = str(chromosome)
        # print(chromosome)
