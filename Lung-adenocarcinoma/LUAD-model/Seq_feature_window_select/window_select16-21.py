...
for w in windows:
    ...
    checkpoint_dir = '/data/lyli/Lung-adenocarcinoma/LUAD-model/Seq_feature_exact/window' + w  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Lung-adenocarcinoma/LUAD-model/Seq_feature_exact/window' + w + '/checkpoints'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    checkpoint_dir = '/data/lyli/Lung-adenocarcinoma/LUAD-model/Seq_feature_exact/window' + w + '/checkpoints/final_seq_model'  # Store model files
    if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
        os.mkdir(checkpoint_dir)

    output_dir = '/data/lyli/Lung-adenocarcinoma/LUAD-model/Seq_feature_exact/window' + w + '/output'  # Store model files
    if not os.path.exists(output_dir):  # if the path does not exist, generate it
        os.mkdir(output_dir)
    filepath = checkpoint_dir + "/LUAD_seq_model_" + str(kkk) + "_mer" + ".hdf5"  # First layer model
    ...
    filename1 = "/data/lyli/Lung-adenocarcinoma/LUAD-enhancers-windows/LUAD-enhancers-300bp/window" + w + "_train_978.txt"
    filename2 = "/data/lyli/Lung-adenocarcinoma/Processed_dataset/nenhancer/NoEnhancer978_300.txt"
    ...
    for pairs in wordVectorSeqList:
        for token in pairs:
            if token not in token_dict:
                ...
                n += 1
    # print(token_dict)
    np.save('/data/lyli/Lung-adenocarcinoma/LUAD-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy', token_dict)
    ...
    # evaluation model
    filename3 = "/data/lyli/Lung-adenocarcinoma/LUAD-enhancers-windows/LUAD-enhancers-300bp/window" + w + "_test_200.txt"
    filename4 = "/data/lyli/Lung-adenocarcinoma/Processed_dataset/nenhancer/NoEnhancer200_300.txt"
    ...
    # Load SeqPoseDict
    token_dict = np.load('/data/lyli/Lung-adenocarcinoma/LUAD-model/Seq_feature_exact/window' + w + '/ModelDict_sequence.npy',
