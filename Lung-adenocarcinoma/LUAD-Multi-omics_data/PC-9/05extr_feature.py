...
with open("/data/lyli/Lung-adenocarcinoma/LUAD-Multi-omics_data/PC-9/H3K4me1-ENCFF487TPY.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Lung-adenocarcinoma/LUAD-Multi-omics_data/PC-9/H3K4me3-ENCFF547MFY.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Lung-adenocarcinoma/LUAD-Multi-omics_data/PC-9/H3K9me3-ENCFF657HCS.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Lung-adenocarcinoma/LUAD-Multi-omics_data/PC-9/H3K27ac-ENCFF735GKU.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Lung-adenocarcinoma/LUAD-Multi-omics_data/PC-9/H3K36me3-ENCFF896YKX.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [550, 500, 200, 150]
featurename = '/data/lyli/Lung-adenocarcinoma/LUAD-Multi-omics_data/PC-9/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Lung-adenocarcinoma/LUAD-enhancers-hg19-1178.fa", "fasta"):
        name = record.id
...
np.savetxt(featurename + 'en_feature_1178_pc9.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
