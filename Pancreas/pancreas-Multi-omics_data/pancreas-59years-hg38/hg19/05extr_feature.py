...
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-59years-hg38/hg19/H3K4me1-ENCFF852WJS.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-59years-hg38/hg19/H3K4me3-ENCFF071KOB.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-59years-hg38/hg19/H3K9me3-ENCFF849YJG.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-59years-hg38/hg19/H3K27ac-ENCFF200MOO.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-59years-hg38/hg19/H3K36me3-ENCFF760YBD.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [400, 900, 300, 650]
featurename = '/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-59years-hg38/hg19/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Pancreas/pancreas_enhancers_3368.fa", "fasta"):
        name = record.id
        ...
        if int(window_extend_size) == 300:
            start_position = start_position - 1200
            end_position = end_position - 1200
            value_one4 = [0]
            value_sum4 = 0
            chromStartlist4 = [0, 0]
            chromEndlist4 = [0, 0]
...
np.savetxt(featurename + 'en_feature_3368_59years.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
