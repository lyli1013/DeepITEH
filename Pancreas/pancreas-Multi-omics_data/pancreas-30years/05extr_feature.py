...
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-30years/H3K4me1-ENCFF289OFU.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-30years/H3K4me3-ENCFF129HJI.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-30years/H3K9me3-ENCFF119CFU.bed")as f:
    for line in f:
...
with open("/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-30years/H3K36me3-ENCFF851NMY.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [150, 650]
featurename = '/data/lyli/Pancreas/pancreas-Multi-omics_data/pancreas-30years/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
np.savetxt(featurename + 'en_feature_3368_30years.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
