...
with open("/data/lyli/Liver-hepatocellular-carcinoma/LIHC-Multi-omics_data/HepG2/H3K4me1-ENCFF448KVF.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Liver-hepatocellular-carcinoma/LIHC-Multi-omics_data/HepG2/H3K4me3-ENCFF943JYO.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Liver-hepatocellular-carcinoma/LIHC-Multi-omics_data/HepG2/H3K9me3-ENCFF306JTV.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Liver-hepatocellular-carcinoma/LIHC-Multi-omics_data/HepG2/H3K27ac-ENCFF276JAI.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Liver-hepatocellular-carcinoma/LIHC-Multi-omics_data/HepG2/H3K36me3-ENCFF037UAP.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [500, 450, 400, 250]
featurename = '/data/lyli/Liver-hepatocellular-carcinoma/LIHC-Multi-omics_data/HepG2/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Liver-hepatocellular-carcinoma/LIHC-enhancers442-hg19.fa", "fasta"):
        name = record.id
...
np.savetxt(featurename + 'en_feature_442_HepG2.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
