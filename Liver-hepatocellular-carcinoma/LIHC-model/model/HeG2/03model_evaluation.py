...
k_merList = [2]  # kmer
kkk = k_merList[0]
BATCH_SIZE = 4   # best_parameter
EPCOHS = 9         # best_parameter
...
checkpoint_dir = '/data/lyli/Liver-hepatocellular-carcinoma/LIHC-model/Bert_LSTM_DNN_model/HeG2/final_model_HeG2'  # Store model files
if not os.path.exists(checkpoint_dir):  # if the path does not exist, generate it
    os.mkdir(checkpoint_dir)

model_weight_filepath = checkpoint_dir + "/LIHC_HeG2_model.h5"  # First layer model
model_structure_filepath = checkpoint_dir + "/LIHC_HeG2_model.json"  # First layer model
...
def Model_DeepITEH(input_seq_length, input_omics_length):
    '''
    Define the classification model
    '''
    def create_lstm_DNN(input_seq_length, input_omics_length, rnn_hidden_dim=RNN_HIDDEN_DIM,
    ...
    return create_lstm_DNN(input_seq_length, input_omics_length)
...
def Model_AUC(model, X_test_first, Y_test_first, BATCH_SIZE):
    ...
    # dataframe.to_csv("/data/lyli/Liver-hepatocellular-carcinoma/LIHC-model/Bert_LSTM_DNN_model/HeG2/LIHC_HeG2_method.csv", index=False, sep=',')
    Test_y_true = np.array(Y_test_first)
    Test_y_scores = np.array(Test_y_pred_score)
    return roc_auc_score(Test_y_true, Test_y_scores)

# Reconstruct the word splitter so that it divides the sequence by 2-mer
...
filename1 = "/data/lyli/Liver-hepatocellular-carcinoma/Processed_dataset-new/enhancer/en_feature_HepG2_RE_AE_test88.txt"  # 测试集
filename2 = "/data/lyli/Liver-hepatocellular-carcinoma/Processed_dataset-new/nenhancer/nen_feature_HepG2_RE_AE_test88.txt"
filename3 = "/data/lyli/Liver-hepatocellular-carcinoma/Processed_dataset-new/enhancer/test_enseq88.txt"
filename4 = "/data/lyli/Liver-hepatocellular-carcinoma/Processed_dataset-new/nenhancer/test_nenseq88.txt"
...
# load bert model
config_path = '/data/lyli/Liver-hepatocellular-carcinoma/LIHC-model/Bert-embedding/cased_L-12_H-768_A-12/bert_config.json'
checkpoint_path = '/data/lyli/Liver-hepatocellular-carcinoma/LIHC-model/Bert-embedding/cased_L-12_H-768_A-12/bert_model.ckpt'
dict_path = '/data/lyli/Liver-hepatocellular-carcinoma/LIHC-model/Bert-embedding/cased_L-12_H-768_A-12/vocab.txt'
model_bert = load_trained_model_from_checkpoint(config_path, checkpoint_path)

# Load SeqPoseDict
token_dict = np.load('/data/lyli/Liver-hepatocellular-carcinoma/LIHC-model/Bert-embedding/lihc_seq_Dict.npy', allow_pickle=True).item()
...
# load model
model_first = Model_DeepITEH(X_seq_test.shape[1], 6)
