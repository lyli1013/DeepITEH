...
def load_omics_feature(filename1, filename2):
    ...
    for file in filelist:
        ...
        for line in file0:
            curLine = list(line)
            floatLine = map(float, curLine)  # The map function is used here to convert the data directly into a float type.
...
X_seq_train = np.loadtxt('/data/lyli/Liver-hepatocellular-carcinoma/LIHC-model/Bert-embedding/LIHC_seq_train_708_wordVector.txt', delimiter=',').reshape((708, 512, 768))
X_seq_train = np.array(X_seq_train)
# Y_train = wordVectorTarList

# load histone modification feature
filename1 = "/data/lyli/Liver-hepatocellular-carcinoma/Processed_dataset/enhancer/en_feature_HepG2_RE_AE_train354.txt"
filename2 = "/data/lyli/Liver-hepatocellular-carcinoma/Processed_dataset/nenhancer/nen_feature_HepG2_RE_AE_train354.txt"
...
if __name__ == '__main__':
    model = Model_DeepITEH(X_seq_train.shape[1], 6)  # Get the first layer model
