...
with open("/data/lyli/Prostate-adenocarcinoma/PRAD-Multi-omics_data/PC-3/H3K4me1-ENCFF312LEI.bed")as f:
    for line in f:
        content_liver_H3K4me1.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K4me3 = []
with open("/data/lyli/Prostate-adenocarcinoma/PRAD-Multi-omics_data/PC-3/H3K4me3-ENCFF010DLV.bed")as f:
    for line in f:
        content_liver_H3K4me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K9me3 = []
with open("/data/lyli/Prostate-adenocarcinoma/PRAD-Multi-omics_data/PC-3/H3K9me3-ENCFF781KAE.bed")as f:
    for line in f:
        content_liver_H3K9me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K27ac = []
with open("/data/lyli/Prostate-adenocarcinoma/PRAD-Multi-omics_data/PC-3/H3K27ac-ENCFF860AQW.bed")as f:
    for line in f:
        content_liver_H3K27ac.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开

content_liver_H3K36me3 = []
with open("/data/lyli/Prostate-adenocarcinoma/PRAD-Multi-omics_data/PC-3/H3K36me3-ENCFF985PEP.bed")as f:
    for line in f:
        content_liver_H3K36me3.append(line.strip().split())    # strip()表示删除掉数据中的换行符，split（‘ ’）则是数据中遇到空格就隔开


window_extend = [400, 500, 200, 300]
featurename = '/data/lyli/Prostate-adenocarcinoma/PRAD-Multi-omics_data/PC-3/Enhancer_feature/'
enhancers_feature = []
H3K4me1_value_sum1 = []
H3K4me3_value_sum2 = []
H3K9me3_value_sum4 = []
...
for window_extend_size in window_extend:
    for record in SeqIO.parse("/data/lyli/Prostate-adenocarcinoma/PRAD-enhancers964-hg19.fa", "fasta"):
        name = record.id
...
np.savetxt(featurename + 'en_feature_964_PC-3.txt', enhancers_feature, encoding='utf_8_sig', delimiter=' ', fmt='%.3f')
